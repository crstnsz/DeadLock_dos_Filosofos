...
public class ThFilosofoComDeadLock extends Thread {
	...
	public void setExecutar(boolean _executar) {this._executar = _executar;}
	public boolean isExecutar() {return _executar;}
	...
	public synchronized void run(){
		while(isExecutar()){
			int tempoAleatorio = (int)(Math.random()*10) + 1;//1 e 10 ms
			try {
			...
			//tempo para comer:
			tempoAleatorio = (int)(Math.random()*51) + 50;//50 e 100 ms
			try {
				this.sleep(tempoAleatorio);
			} catch (InterruptedException e) {
				e.printStackTrace();
